...
export COOKIE PI_IP GATEWAY_IP LAN_NET_PREFIX QUERY_LIMIT MIN_QUERIES MIN_COVERAGE BYPASS_CHECK ADGUARD_WEB_PORT
python3 - <<'PY'
...
for ip in sorted(active):
    print(f"  [OK] {ip}: {clients[ip]} sorgu, {blocked.get(ip, 0)} engel")
for ip in low:
    tag = "IDLE" if ip in idle_ips else "WARN"
    print(f"  [{tag}] {ip}: {clients[ip]} sorgu (cok az — DHCP yenile veya Private DNS kapat)")
for ip in missing_strict:
    icmp = "ping-ok" if ip in verified_active else "ICMP-kapali/filtre"
    print(f"  [BYPASS] {ip} ({mac_for(ip)}, {icmp}): ARP aktif, Pi DNS yok")
for ip in missing_idle:
    print(f"  [IDLE] {ip} ({mac_for(ip)}): STALE ARP, query logda yok")

print("")
print("=== Olası nedenler (modem DNS tek basina yetmez) ===")
print("  1. Modemde WAN DNS degil LAN/DHCP DNS ayarlanmali")
print("  2. Ikincil DNS (8.8.8.8) bos olmali")
...
print("  B) Agresif: NETWORK_MODE=adguard-dhcp (modem DHCP kapat) — docs/ADGUARD-DHCP.md")
print("  C) Modem guvenlik duvarinda dis DNS (53) engeli varsa ac")
