...
| Service | URL | Role |
|---------|-----|------|
| Forgejo | http://git.home or `http://PI_STATIC_IP:3002` | Git server |
| Syncthing | http://sync.home or `http://PI_STATIC_IP:8384` | Mac ↔ Pi file sync |
| Restic | (background) | Daily backup on SSD |

`PI_STATIC_IP` = LAN IP in `.env`.

## Forgejo first-time setup

1. Browser: http://git.home or `http://PI_STATIC_IP:3002`
2. Create admin user on first launch
3. Add remote from Mac:
   ```bash
   git remote add pi http://git.home/user/repo.git
   git push pi main
   ```
...
- UI: http://sync.home or `http://PI_STATIC_IP:8384`
