...
for unit in pi-gateway-health.timer pi-gateway-backup.timer pi-gateway-crowdsec-ufw.timer pi-gateway-adguard-filters.timer pi-data-symlink.timer pi-ssd-watch.path pi-ssd-health.timer pi-gateway-ssd-smart.timer pi-gateway-kuma-report.timer pi-gateway-speedtest.timer pi-gateway-quake.timer pi-gateway-ibb.timer; do
...
sudo systemctl start pi-gateway-health.timer pi-gateway-backup.timer pi-gateway-adguard-filters.timer pi-gateway-speedtest.timer pi-gateway-quake.timer pi-gateway-ibb.timer 2>/dev/null || true
if [[ "$STORAGE_TYPE" == "hybrid" || "$STORAGE_TYPE" == "ssd-data" ]]; then
