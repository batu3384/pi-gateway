...
status_card="$ROOT/scripts/lib/telegram-status-card.py"
...
grep -q '📋 Bülten' "$tmpl" || die "bülten iskeleti yok"
! grep -q 'Pi Gateway · Bülten\|Pi Gateway · Ağ\|Pi Gateway · Zamanlanmış görev' "$tmpl" \
  || die "bülten/cron branding kaldi"
...
! grep -q '📋 Pi Gateway ·\|Pi Gateway yeniden\|Pi Gateway kısa' "$patch" \
  || die "cron patch branding kaldi"
! grep -q 'Firecrawl 403\|patch-hermes-config-pi\|Ağ Gözcüsü' "$patch" \
  || die "v7 jargon/betik adi kalmis"
ok "cron patch v7"

workflow="$ROOT/config/n8n/uptime-kuma-alert.workflow.json"
! grep -q 'Pi Gateway · İzleme\|Servis yeniden çalışıyor\|Servis yanıt vermiyor' "$workflow" \
  || die "Kuma mesaj tekrarı/branding kaldi"
ok "Kuma mesaj kopyası"
! grep -q 'Pi Gateway · Durum\|<b>Pi Gateway</b>' "$status_card" "$panels" \
  || die "durum/panel branding kaldi"
ok "durum kartı kopyası"
...
grep -q '📊 Piyasa' "$fx" || die "fx piyasa başlığı yok"
! grep -q 'Pi Gateway ·\|^ *Pi Gateway$' "$fx" || die "fx branding kaldi"
