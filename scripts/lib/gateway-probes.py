...
def _who_home() -> tuple[int, str]:
    ...
    try:
        ...
        return len(names), ", ".join(names[:12])
    except Exception:
        return 0, ""


def probe() -> dict[str, Any]:
    dns_ms = _dns_query_ms("127.0.0.1")
...
def self_check() -> None:
    ...
    lines = prom_lines({"dns_latency_ms": 12, "panel_latency_ms": 80, "hosts_online": 3})
