...
- **Runtime recovery (software):** `ssd_mount_healthy` (timed write + `fsync`) detects hung mounts. `ssd_usb_soft_reset` merdiven: LPM/autosuspend off → lazy umount → usb-storage unbind/bind → USB3 xhci port tarama (Pi4 `platform/scb/.../usb2-portN`, `SSD_USB_PORT_SCAN_MAX=8`) → opt-in xHCI. Kota bir tarama turu = 1. `pi-ssd-health.timer` 30s sahip; `health-check` gozlem-only. udev `add|remove` → `pi-ssd-watch`. Degraded = core-dns; restore = remount + symlink + full stack.
